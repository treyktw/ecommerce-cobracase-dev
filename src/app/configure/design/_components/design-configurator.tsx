...
import { COLORS, FINISHES, MATERIAL, MODELS } from "@/validators/options-validator"
...
const DesignConfigurator = ({ configId, imageUrl, imageDimensions }: DesignConfiguratorProps) => {
  ...
  async function saveConfiguration () {
    try {
      ...
      const leftOffest = caseLeft - containerLeft;
      const topOffset = caseTop - containerTop;

      const acutalX = renderedPosition.x - leftOffest;
    ...
    <div className="relative mt-20 grid lg:grid-cols-3 grid-cols-1 mb-20 pb-20">
      <div className='relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2' ref={containerRef}>
        <div className='relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]'>
          ...
          <div className={cn("absolute inset-0 top-px rounded-[32px] right-[3px] bottom-px", `bg-${options.color.tw}`)} />
        </div>
        <Rnd
          default={{
            x: 150,
          ...
          onResizeStop={(_, __, ref, ___, { x, y }) => {
            setRenderedDimension({
              ...
              width: parseFloat(ref.style.width.slice(0, -2))
            })
          ...
          <div className="px-8 pb-12 pt-8 ">
            ...
            <div className="relative mt-4 h-full flex flex-col justify-between">
              <div className="flex flex-col gap-6">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      {MODELS.options.map((model) => (
                        <DropdownMenuItem key={model.label} className={cn("flex text-sm flex-1 items-center p-1.5 cursor-default hover:bg-zinc-100", {
                ...
                {[MATERIAL, FINISHES].map(({name, options: selectableOptions, }) => (
                      ...
                      {selectableOptions.map((option) => (
                        <Radio key={option.value} value={option} className={({ checked }) => cn("relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:outline-none ring-0 focus:ring-0 outline-none sm:flex sm:justify-between", {
                          ...
                          <Description as={"span"} className="mt-2 flex text-sm sm:ml-4 sm:mt-0 sm:flex-col sm:text-right">
                            <span className="font-md text-gray-900">
